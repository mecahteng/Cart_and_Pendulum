...
subplot(2, 2, 2);
for i = 1:length(t)
    draw_pendulum_trajectory(x_solution(i), phi_solution(i), l, (i-1) / length(t));
...
function [c, ceq] = constraint(coeffs, M, m, l, g)
    % Constraint function to enforce zero velocity at the final time

    % Simulate the system with the current coefficients
    [t, state] = ode45(@(t, state) dynamics(t, state, M, m, l, g, @(t) force_input(t, coeffs)), [0 2], [0; 0; 0; 0]);

    % Extract velocities at the final time
    x_dot_tip_final = state(end, 3);
    phi_dot_tip_final = state(end, 4);

    % Constraints
    c = [];
    ceq = [x_dot_tip_final; phi_dot_tip_final];  % Enforce zero velocity at the tip at the final time
